...
import { BrainCircuit, MessageSquare, History, Send, Clock, Key, Download, BarChart3, FileUp, LogOut, User as UserIcon } from 'lucide-react';
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-900 font-sans p-4">
        <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md">
          <div className="flex flex-col items-center mb-8">
            <div className="bg-indigo-600 p-3 rounded-2xl mb-4"><BrainCircuit className="w-10 h-10 text-white" /></div>
            <h1 className="text-3xl font-black text-slate-900">ANTON</h1>
            <p className="text-slate-500 text-sm">Artificial Intelligence Insight Engine</p>
          </div>
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <button className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition-all uppercase tracking-widest">
              {authMode === 'login' ? 'Enter Engine' : 'Create Identity'}
            </button>
          </form>
          <p className="text-center mt-6 text-sm text-slate-500">
            {authMode === 'login' ? "New to the system?" : "Already recognized?"}
            <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="ml-2 text-indigo-600 font-bold uppercase text-xs">
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <header className="flex items-center justify-between pb-6 border-b border-slate-200">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-4xl font-black tracking-tighter text-slate-900 uppercase">ANTON</h1>
          </div>
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 px-4 py-2 bg-white border border-slate-200 rounded-full shadow-sm">
        ...
        {/* ... Rest of the UI remains the same (Input Section, Chart, History) ... */}
                ...
                {history.map((item) => (
                  <div key={item.id} className="group p-4 bg-slate-50 border border-slate-100 rounded-xl hover:border-indigo-200 transition-all">
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <button onClick={() => {
                            const blob = new Blob([`ANTON REPORT\n\nTEXT: ${item.original_text}\nSENTIMENT: ${item.sentiment_label}`], {type: 'text/plain'});
                            const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `Report_${item.id}.txt`; a.click();
                        }} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-indigo-600 transition-all"><Download className="w-4 h-4" /></button>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <div className="flex items-center gap-1 px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded text-[10px] font-bold"><Clock className="w-3 h-3" /> {item.reading_time}m</div>
                    </div>
                  </div>
                ))}
              </div>
